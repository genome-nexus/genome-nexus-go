...
func GetVariantAnnotations(genomicLocations []pubgn.GenomicLocation) []pubgn.VariantAnnotation {
	...
	if err != nil {
		log.Printf("Error when calling `AnnotationControllerApi.FetchVariantAnnotationByGenomicLocationGET``: %v\n", err)
		log.Printf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	...
	for i := 0; i < len(variantAnnotations); i++ {
		if !*variantAnnotations[i].SuccessfullyAnnotated {
			log.Printf("ERROR: Failed to annotate: ", variantAnnotations[i].Variant)
		} else {
			log.Printf("Success: ", *variantAnnotations[i].SuccessfullyAnnotated)
		}
	}
